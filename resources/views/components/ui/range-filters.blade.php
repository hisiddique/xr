...
@php
    $hasAny = $dateFrom !== '' || $dateTo !== '' || $amountMin !== '' || $amountMax !== '' || $extraHasValue;
@endphp
...
<div class="flex flex-col gap-2 sm:flex-row sm:flex-wrap sm:items-end">
    ...
    <div class="flex flex-col">
        <label class="mb-0.5 text-xs font-semibold uppercase tracking-wider text-zinc-400 dark:text-zinc-500">{{ __('Min :cur', ['cur' => $currency]) }}</label>
        <flux:input wire:model.live.debounce.400ms="amountMin" type="number" step="0.01" min="0" size="sm" placeholder="0.00" class="!w-28" />
    </div>
    <div class="flex flex-col">
        <label class="mb-0.5 text-xs font-semibold uppercase tracking-wider text-zinc-400 dark:text-zinc-500">{{ __('Max :cur', ['cur' => $currency]) }}</label>

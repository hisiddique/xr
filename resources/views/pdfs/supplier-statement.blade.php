...
@php
    ...
    $supplierPerson = $supplier ? trim(
        ($supplier->title?->name ?? '').' '.
        ($supplier->first_name ?? '').' '.
        ($supplier->last_name ?? '')
    ) : '';
    $supplierAddressLines = $supplier ? collect([
...
{{-- Fixed page header: company chrome + supplier/statement-meta bands --}}
                ...
                <td>
                    <table class="split"><tr>
                        ...
                        <td class="right meta-right">
                            ...
                            <table class="kv">
                                <tr><td class="k">Account</td><td class="c">:</td><td>{{ $supplier->reference ?? '—' }}</td></tr>
    ...
    <tbody>
        @foreach($invoices as $invoice)
            ...
            <tr>
                ...
                <td class="item-cell">
                    @if($rowType === 'debit_note')
                        Debit Note {{ $invoice['doc_number'] }}
                    @elseif($rowType === 'payout')
                        Payout {{ $invoice['doc_number'] }}
                    @else

...
new class extends Component {
    ...
    {
        $email = $this->debitNote->supplier->email ?? '';
        if ($email) {
            $this->emails = [$email];
        }
    }
    ...
    public function send(): void
    {
        ...
        try {
            app(SupplierDebitNoteEmailService::class)->send(
                $this->debitNote,
                $this->emails,
                $this->notes ?: null,
            );
            ...
            Flux::modal('email-supplier-debit-note-'.$this->debitNote->id)->close();
...
<div>
    <flux:modal name="email-supplier-debit-note-{{ $debitNote->id }}" focusable class="max-w-md">
        <div class="space-y-6">
            <div>
                <flux:heading size="lg">{{ __('Send Debit Note') }}</flux:heading>
                <flux:subheading>
                    {{ __('Send :reference to the supplier by email.', ['reference' => $debitNote->reference]) }}
                </flux:subheading>
            </div>

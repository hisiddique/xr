...
new #[Title('Customer Categories')] class extends Component {
    ...
    public function addCategory(): void
    {
        ...
        $this->validateOnly('newCategory', ['newCategory' => 'required|string|max:50|unique:lookup_customer_categories,name']);
        LookupCustomerCategory::create(['name' => $this->newCategory]);
        $this->newCategory = '';
    ...
    public function deleteCategory(): void
    {
        ...
        LookupCustomerCategory::findOrFail($this->deletingCategoryId)->delete();
    ...
    #[Computed]
    public function deletingCategory(): ?LookupCustomerCategory
    {
        return $this->deletingCategoryId
            ? LookupCustomerCategory::find($this->deletingCategoryId)
            : null;
    }
}; ?>
...
<div class="flex flex-col gap-8">
    ...
    <x-ui.page-header
        ...
        subtitle="Retail, Wholesale, Trade, etc."
    />
    ...
    <div class="max-w-xl">
        <div class="overflow-hidden rounded-2xl border border-zinc-200/70 bg-white shadow-[0_1px_2px_rgba(16,24,40,0.06),0_1px_3px_rgba(16,24,40,0.10)] dark:border-white/10 dark:bg-zinc-900">
            <div class="border-b border-zinc-200/70 px-6 py-4 dark:border-white/10">
                <h2 class="text-sm font-semibold text-zinc-900 dark:text-white">Customer Categories</h2>
                <p class="text-xs text-zinc-500 dark:text-zinc-400">Categories used to classify customers</p>
            </div>
            <div class="border-b border-zinc-100 px-6 py-4 dark:border-white/[0.06]">
                <form wire:submit="addCategory" class="flex gap-2">
                    <flux:input wire:model="newCategory" :placeholder="__('e.g. Wholesale')" maxlength="50" class="flex-1" data-add-input />
                    <flux:button type="submit" variant="primary" icon="plus">Add</flux:button>
                </form>
                <flux:error name="newCategory" />
            </div>
            @if($this->categories->isEmpty())
                <x-ui.empty-state icon="tag" title="No categories yet" description="Add your first customer category above." />
            @else
                <table class="w-full">
                    <thead>

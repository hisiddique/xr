...
new #[Title('Customer Groups')] class extends Component {
    ...
    public function addGroup(): void
    {
        ...
        $this->validate([
            'newGroupName' => 'required|string|max:80|unique:customer_groups,name',
    ...
    public function updateGroup(): void
    {
        ...
        $this->validate([
            'editingGroupName' => 'required|string|max:80|unique:customer_groups,name,'.$this->editingGroupId,
            'editingGroupDescription' => 'nullable|string|max:255',
        ]);

        CustomerGroup::findOrFail($this->editingGroupId)->update([
    ...
    public function deleteGroup(): void
    {
        ...
        CustomerGroup::findOrFail($this->deletingGroupId)->delete();
    ...
    public function addMembers(array $ids): void
    {
        ...
        CustomerGroup::find($this->selectedGroupId)?->customers()->syncWithoutDetaching($ids);

        Flux::modal('add-members')->close();
        Flux::toast(variant: 'success', text: __(':n customer(s) added to the group.', ['n' => count($ids)]));
    }
    ...
    public function removeMembers(): void
    {
        ...
        CustomerGroup::find($this->selectedGroupId)?->customers()->detach($this->removingMemberIds);
    ...
    {
        return CustomerGroup::withCount('customers')->orderBy('name')->get();
    }

    #[Computed]
    public function selectedGroup(): ?CustomerGroup
    {
        return $this->selectedGroupId ? CustomerGroup::find($this->selectedGroupId) : null;
    }

    #[Computed]
    public function members(): Collection
    {
        return $this->selectedGroup?->customers()->orderBy('company_name')->get() ?? collect();
    }
    ...
    public function nonMembersForPicker(): array
    {
        ...
        return Customer::query()
            ->when($this->selectedGroup, fn ($q) => $q->whereNotIn('id', $this->selectedGroup->customers()->pluck('customers.id')))
            ->orderBy('company_name')
            ->get(['id', 'company_name', 'reference'])
            ->map(fn ($c) => ['id' => (string) $c->id, 'label' => $c->company_name, 'ref' => (string) $c->reference])
            ->all();
    }

    #[Computed]
    public function editingGroup(): ?CustomerGroup
    {
        return $this->editingGroupId ? CustomerGroup::find($this->editingGroupId) : null;
    }

    #[Computed]
    public function deletingGroup(): ?CustomerGroup
    {
        return $this->deletingGroupId ? CustomerGroup::find($this->deletingGroupId) : null;
    }
}; ?>
...
<div class="flex flex-col gap-8">
    ...
    <x-ui.page-header
        ...
        subtitle="Mailing lists of customers for statement dispatch"
    />
    ...
    <div class="grid gap-8 lg:grid-cols-2">
        ...
        <div class="overflow-hidden rounded-2xl border border-zinc-200/70 bg-white shadow-[0_1px_2px_rgba(16,24,40,0.06),0_1px_3px_rgba(16,24,40,0.10)] dark:border-white/10 dark:bg-zinc-900">
            <div class="flex items-center justify-between gap-2 border-b border-zinc-200/70 px-6 py-4 dark:border-white/10">
                <div>
                    <h2 class="text-sm font-semibold text-zinc-900 dark:text-white">Customer Groups</h2>
                    <p class="text-xs text-zinc-500 dark:text-zinc-400">Groups used to dispatch statements to customers</p>
                </div>
                <flux:button size="sm" variant="primary" icon="plus" x-on:click="$flux.modal('create-group').show()">New group</flux:button>
            </div>
            @if($this->groups->isEmpty())
                <x-ui.empty-state icon="user-group" title="No groups yet" description="Create your first customer group." />
            @else
                <table class="w-full">
                    <thead>
                    ...
                    <tbody class="divide-y divide-zinc-50 dark:divide-white/[0.04]">
                        @foreach($this->groups as $group)
                            <tr @class(['bg-indigo-50 dark:bg-indigo-500/10' => $selectedGroupId === $group->id])>
                                ...
                                <td class="px-6 py-3">
                                    <flux:badge size="sm">{{ $group->customers_count }}</flux:badge>
                                </td>
        ...
        <div class="overflow-hidden rounded-2xl border border-zinc-200/70 bg-white shadow-[0_1px_2px_rgba(16,24,40,0.06),0_1px_3px_rgba(16,24,40,0.10)] dark:border-white/10 dark:bg-zinc-900">
            ...
            @if($this->selectedGroup)
                ...
                @if($this->members->isEmpty())
                    <div class="px-6 py-4 text-sm text-zinc-500 dark:text-zinc-400">No customers in this group yet.</div>
                @else
    ...
    <flux:modal name="create-group" focusable class="max-w-sm" @close="$wire.set('newGroupName', ''); $wire.set('newGroupDescription', '')">
        <form wire:submit="addGroup" class="space-y-4">
            <flux:heading>{{ __('New customer group') }}</flux:heading>
            <div>
        ...
        <div class="space-y-4">
            <flux:heading>
                @if($this->deletingGroup)
                    {{ __('Delete ":name"? Members are detached, customers are not deleted.', ['name' => $this->deletingGroup->name]) }}
                @endif
            </flux:heading>
            <div class="flex justify-end gap-2">
                <flux:modal.close><flux:button variant="filled">Cancel</flux:button></flux:modal.close>
    ...
    <flux:modal name="add-members" focusable class="max-w-lg">
        <div
            ...
            class="space-y-4"
        >
            <flux:heading>{{ __('Add customers to :group', ['group' => $this->selectedGroup?->name]) }}</flux:heading>
            ...
            <div class="max-h-80 divide-y divide-zinc-100 overflow-y-auto rounded-lg border border-zinc-200 dark:divide-white/[0.06] dark:border-white/10">
                ...
                <p x-show="filtered.length === 0" class="px-3 py-4 text-sm text-zinc-500 dark:text-zinc-400">{{ __('No matching customers.') }}</p>
            </div>
    ...
    <flux:modal name="remove-members" focusable class="max-w-sm" @close="$wire.set('removingMemberIds', [])">
        <div class="space-y-4">
            ...
            <p class="text-sm text-zinc-500 dark:text-zinc-400">{{ __('The customers are not deleted — only their membership.') }}</p>
            <div class="flex justify-end gap-2">
                <flux:modal.close><flux:button variant="filled">Cancel</flux:button></flux:modal.close>

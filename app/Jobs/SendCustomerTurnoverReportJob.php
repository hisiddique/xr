...
use App\Mail\CustomerTurnoverReportMail;
...
class SendCustomerTurnoverReportJob implements ShouldQueue
{
    ...
    public function handle(CustomerTurnoverReportService $service): void
    {
        ...
        try {
            ...
            foreach ($files as $file) {
                if ($withinCap) {
                    $attachments[] = [
                        ...
                        'filename' => 'customer-turnover.'.$file['format'],
            ...
            $customerCount = $files[0]['customerCount'] ?? 0;
            $total = $files[0]['total'] ?? 0.0;

            Mail::to($this->emails)->send(new CustomerTurnoverReportMail(
                $customerCount,
                $total,
                $this->notes,
                $attachments,
     ...
     *
     * @return array<int, array{format: string, path: string, size: int, exportJob: ExportJob, customerCount: int, total: float}>
     */
    private function generateFiles(CustomerTurnoverReportService $service, ExportJob $export): array
    {
        $files = [];
        ...
        foreach ($this->formats as $format) {
            $fileExport = ExportJob::create([
                ...
                'rows_total' => $service->customersQuery($export->filters ?? [])->count(),
                'started_at' => now(),
                'created_by' => $export->created_by,
            ]);

            $path = 'exports/'.$fileExport->id.'/customer-turnover.'.$format;
            ...
            try {
                ...
                $fileExport->update([
                    ...
                    'download_path' => $path,
                    'rows_processed' => $result['customerCount'],
                    'finished_at' => now(),
                ]);
            } catch (\Throwable $e) {

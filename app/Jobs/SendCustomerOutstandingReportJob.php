...
class SendCustomerOutstandingReportJob implements ShouldQueue
{
    ...
    public function handle(CustomerOutstandingReportService $service): void
    {
        ...
        try {
            ...
            foreach ($files as $file) {
                if ($withinCap) {
                    $attachments[] = [
                        ...
                        'filename' => 'customer-outstanding-payments.'.$file['format'],
            ...
            $totalOutstanding = $files[0]['totalOutstanding'] ?? 0.0;

            Mail::to($this->emails)->send(new CustomerOutstandingReportMail(
                $customerCount,
                $totalOutstanding,
                $this->notes,
                $attachments,
     ...
     *
     * @return array<int, array{format: string, path: string, size: int, exportJob: ExportJob, customerCount: int, totalOutstanding: float}>
     */
    private function generateFiles(CustomerOutstandingReportService $service, ExportJob $export): array
    {
        $files = [];
        ...
        foreach ($this->formats as $format) {
            ...
            $path = 'exports/'.$fileExport->id.'/customer-outstanding-payments.'.$format;
